...
static int saveState()
{
  ...
  return EXIT_SUCCESS;
  #else
  std::cerr << "Error, please rebuild with HDF5 support before saving state." << std::endl;
  return EXIT_FAILURE;
  #endif
}
...
static int stepSystem()
{
  ...
  if( g_output_forces && g_iteration % g_steps_per_save == 0 )
  {
    ...
    #else
    std::cerr << "Error, please rebuild with HDF5 support before saving forces." << std::endl;
    return EXIT_FAILURE;
    #endif
  }
