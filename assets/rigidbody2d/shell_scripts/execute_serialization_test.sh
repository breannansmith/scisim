...
if [ "$#" -ne 5 ]; then
  ...
  echo "Usage:" $0 "xml_file_name end_time compare_frame_number resume_frame_number output_fps"
  exit 1
fi
...
if ! [[ "$output_fps" =~ $integerregexp ]] ; then
  echo "Error, fifth argument must be a non-negative integer. Exiting." >&2; exit 1
fi
...
echo "Executing full simulation run: ./rigidbody2d_cli $xml_file_name -f $output_fps -s 1 -e $end_time -o $output_directory -i > /dev/null"
./rigidbody2d_cli $xml_file_name -f $output_fps -s 1 -e $end_time -o $output_directory -i > /dev/null
if [ $? -ne 0 ] ; then
...
echo "Resuming from intermediate state: ./rigidbody2d_cli -r $output_directory/serial_$resume_frame.bin > /dev/null"
./rigidbody2d_cli -r $output_directory/serial_$resume_frame.bin > /dev/null
if [ $? -ne 0 ] ; then
...
echo "Comparing final config data: h5diff --exclude-path \"/run_stats\" $output_directory/config_final_output.h5 $output_directory/config_$compare_frame.h5"
#h5diff --exclude-path "/run_stats" $output_directory/config_final_output.h5 $output_directory/config_$compare_frame.h5
