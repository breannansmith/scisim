...
echo "Executing full simulation run: ./ball2d_cli $xml_file_name -s 1 -e $end_time -o $output_directory -f $output_frame_rate -i > /dev/null"
./ball2d_cli $xml_file_name -s 1 -e $end_time -o $output_directory -f $output_frame_rate -i > /dev/null
if [ $? -ne 0 ] ; then
...
echo "Resuming from intermediate state: ./ball2d_cli -r $output_directory/serial_$resume_frame.bin > /dev/null"
./ball2d_cli -r $output_directory/serial_$resume_frame.bin > /dev/null
if [ $? -ne 0 ] ; then
