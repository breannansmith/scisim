#include "RigidBody2DSobogusInterface.h"
...
RigidBody2DSobogusInterface::RigidBody2DSobogusInterface()
: m_primal( nullptr )
, m_dual( nullptr )
{}
...
void RigidBody2DSobogusInterface::fromPrimal( const unsigned num_bodies, const Eigen::VectorXd& masses, const Eigen::VectorXd& f_in, const unsigned num_contacts, const Eigen::VectorXd& mu, const Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic,Eigen::ColMajor>& contact_bases, const Eigen::VectorXd& w_in, const Eigen::VectorXi& obj_a, const Eigen::VectorXi& obj_b, const Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor>& HA, const Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor>& HB )
{
  ...
  m_primal->M.setRows( num_bodies, 3 );
  m_primal->M.setCols( num_bodies, 3 );
  for( unsigned bdy_idx = 0; bdy_idx < num_bodies; ++bdy_idx )
  {
    m_primal->M.insertBack( bdy_idx, bdy_idx ) = Eigen::MatrixXd::Map( &masses( 9 * bdy_idx ), 3, 3 );
  }
  m_primal->M.finalize();
  ...
  m_primal->H.setCols( num_bodies, 3 );
  ...
  for( unsigned cntct_idx = 0; cntct_idx < num_contacts; ++cntct_idx )
  {
    ...
    {
      m_primal->H.insert( cntct_idx, obj_a( cntct_idx ) ) =   HA.block<2,3>( 2 * cntct_idx, 0 );
      m_primal->H.insert( cntct_idx, obj_b( cntct_idx ) ) = - HB.block<2,3>( 2 * cntct_idx, 0 );
    }
    ...
    {
      std::cerr << "Error, impossible code path hit in RigidBody2DSobogusInterface::fromPrimal. This is a bug." << std::endl;
...
double RigidBody2DSobogusInterface::solve( Eigen::VectorXd& r, Eigen::VectorXd& v, unsigned& num_iterations, const unsigned max_threads, const double& tol, const unsigned max_iters, const unsigned eval_every, const bool use_infinity_norm )
{
  assert( m_primal );
...
double RigidBody2DSobogusInterface::evalInfNormError( const Eigen::VectorXd& r )
{
  if( !m_dual )
  {
    computeDual();
